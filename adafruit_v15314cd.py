...
class VL53L4CD:
    ...
    def _write_register(self, address, data, length=None):
        if length is None:
            length = len(data)
        self.i2c.writeto(self.chip,struct.pack(">H", address) + data[:length])

    def _read_register(self, address, length=1):
        data = bytearray(length)
        self.i2c.writeto(self.chip,struct.pack(">H", address))
        self.i2c.readfrom_into(self.chip,data,length)
        return data
